...
void USART3_IRQHandler(void)                	//串口3中断服务程序
{
	...
	if(USART_GetITStatus(USART3, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
	{
		...
		if(rxBuf == '1')  //向前：舵机5加，舵机4加
		{
			if(ServoPwmDutySet[5] < 2500) ServoPwmDutySet[5] += 20;
			if(ServoPwmDutySet[4] > 500) ServoPwmDutySet[4] -= 20;
			ServoSetPluseAndTime(5, ServoPwmDutySet[5], 50);
			ServoSetPluseAndTime(4, ServoPwmDutySet[4], 50);
		}
		else if(rxBuf == '2')  //向后：舵机5减，舵机4减
		{
			if(ServoPwmDutySet[5] > 500) ServoPwmDutySet[5] -= 20;
			if(ServoPwmDutySet[4] < 2500) ServoPwmDutySet[4] += 20;
			ServoSetPluseAndTime(5, ServoPwmDutySet[5], 50);
			ServoSetPluseAndTime(4, ServoPwmDutySet[4], 50);
		}
		else if(rxBuf == '3')  //向左：舵机6减
		{
			if(ServoPwmDutySet[6] > 500) ServoPwmDutySet[6] -= 20;
			ServoSetPluseAndTime(6, ServoPwmDutySet[6], 50);
		}
		else if(rxBuf == '4')  //向右：舵机6加
		{
			if(ServoPwmDutySet[6] < 2500) ServoPwmDutySet[6] += 20;
			ServoSetPluseAndTime(6, ServoPwmDutySet[6], 50);
		}
		else if(rxBuf == '5')  //向上：舵机3减
		{
			if(ServoPwmDutySet[3] < 2500) ServoPwmDutySet[3] += 20;
			ServoSetPluseAndTime(3, ServoPwmDutySet[3], 50);
		}
		else if(rxBuf == '6')  //向下：舵机3加
		{
			if(ServoPwmDutySet[3] > 500) ServoPwmDutySet[3] -= 20;
			ServoSetPluseAndTime(3, ServoPwmDutySet[3], 50);
		}
		if(!fFrameStart)
		{
